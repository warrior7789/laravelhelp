    ...
    <div class="page-content container-fluid seboX">
        ...
        <div class="row">
            <div class="col-md-12 skill-table">
                {{ Form::model($model, array('route' => array('admin.spskill.store') ,'files' => true , 'class' => 'row' )) }}
                {{ Form::hidden('id') }}
                <input type="hidden" name="user_id" value="{{ $userid }}">
                <div class="col-md-6">
                    {{-- <div class="form-group {{ $errors->has('user_id') ? 'has-error' : ''}}">
                        <label for="user_id">@lang('strings.new.select_user')</label>
                        {{ Form::select('user_id', $user, $value = null, $attributes = array('class' => 'form-control','id'=>'user_id','required'=>'required') ) }}
                    ...
                    <div class="form-group {{ $errors->has('tags') ? 'has-error' : ''}}">
                        <label for="tags">@lang('strings.new.tags')</label>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group {{ $errors->has('offer_img') ? 'has-error' : ''}}">
                        ...
                        @if(!empty($model->offer_img))
                            <img style="height: 100px;width: 100px;" src="/storage/spskills/{{ $model->offer_img }}">
                        @endif
                    </div>
                    ...
                    <a href="{{ route('admin.serviceprovider.edit',$userid) }}" class="btn btn-danger  upd-btn"> @lang('strings.new.cancel')</a>
                </div>
                {{ Form::close() }}
            </div>
        </div>
