...
trait UserAttribute
{
    ...
    {
        return '<a href="'.route('admin.auth.user.destroy', $this).'" name="confirm_item" class="btn btn-danger"><i class="fas fa-trash" data-toggle="tooltip" data-placement="top" title="'.__('buttons.backend.access.users.delete_permanently').'"></i></a> ';
  ...
  //   	<div class="btn-group" role="group" aria-label="'.__('labels.backend.access.users.user_actions').'">
		...
		//   </div>
		// </div>';
        if($this->login_as_button){
          $delete = $this->delete_permanently_button;
        }else{
          $delete = '';
        }
        return '
