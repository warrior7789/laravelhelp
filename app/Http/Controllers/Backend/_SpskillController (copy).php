...
class SpskillController extends Controller
{
    ...
    public function Create(Request $request ,$id='' ) {
        ...
        Breadcrumbs::for('admin.spskill.create', function ($trail) {
            $trail->push(__('strings.new.spskill'), route('admin.spskill.create'));
        });
        
        return view('backend.spskill.create',compact('model','id','skills','Currency','user'));
    }
    ...
    public function Store(Request $request){
        ...
        $rules = [
            ...
            'price_per_hour'    => 'required|numeric|min:1|max:200',
            'price_per_day'     => 'required|numeric|min:1|max:200',
            'show_price'        => 'required',
        ...
        $Spskill->price_per_hour= $request->get('price_per_hour');
        $Spskill->price_per_day = $request->get('price_per_day');
        ...
        if(!empty($request->get('id')))      
            return redirect()->route('admin.spskill')->with('success', __('strings.new.skill_insert_message'));

        return redirect()->route('admin.spskill')->with('success',__('strings.new.skill_update_message'));
    ...
    {   
        ...
        return redirect()->route('admin.spskill')
                        ->with('success',__('strings.new.delete_message'));
